...
def _to_form_value(value: object) -> str | list[str]:
...
class ElevenLabsAudioTranscriptionConfig(BaseAudioTranscriptionConfig):
    ...
    def transform_audio_transcription_request(
        ...
        # Prepare form data
        # Values may be str OR list[str] (see _to_form_value); Final forbids
        # rebinding, not mutation, so the per-key assignments below are fine.
        form_data: Final[dict] = {"model_id": model}
    ...
    def transform_audio_transcription_response(
        ...
        try:
            ...
            # Pass it through verbatim so stereo callers can attribute words to a
            # speaker by channel; without this the per-channel data is dropped.
